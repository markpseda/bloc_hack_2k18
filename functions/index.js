...
exports.uploadFile = functions.storage.bucket(mybucket).object().onFinalize((object, context) => {
    ...
    let metaToken = object.metadata['firebaseStorageDownloadTokens'];
    let downloadURL = `https://firebasestorage.googleapis.com/v0/b/priv-storage.appspot.com/o/files%2F${file}?alt=media&token=${metaToken}`;
    console.log(downloadURL);
    db.ref(`documents/${file}`).update(
        {
            downloadURL
        }
...
exports.onUpload = functions.database.ref(`a-user/{userID}/onUpload`)
    .onCreate((snap, context) => {
        ...
        let saltNum = 8; //rand num btw 3 and 12
        let i = saltNum;
        while(i > 0){
            hashedPass = SHA512(hashedPass);
            i--;   
        }
        db.ref(`passwords/${uniqueID}`).update(
            {
                ...
                (error)=>{
                    ...
                    db.ref(`a-user/${userID}/onUpload`).remove();
                }
            );
    });
...
exports.onDownload = functions.database.ref(`a-user/{userID}/onDownload`)
    .onCreate((snap, context) => {
        ...
        let hashedPass = data.hashedPass;
        let secret = {};
        db.ref(`passwords/${uniqueID}`).once('value').then(function(snapshot){
            secret = snapshot.val();
        }).then(
            (success)=>{
                console.log(secret);
                ...
                if(!(hashedPass === secret['hashedPass'])){
                    ...
                    console.log("match");
                    let dURL;
                    db.ref(`documents/${uniqueID}/downloadURL`).once('value').then(function(snapshot){
                        dURL = snapshot.val();
                    }).then(
                        (success)=>{
                            console.log("downloading");
                            db.ref(`a-user/${userID}/onDownload/`).update(
                                {
                                    'downloadURL':dURL
                                }
                            );
                ...
                if(!(hashedPass === secret['hashedPass'])){
                    ...
                    db.ref(`passwords/${uniqueID}`).update(
                        ...
                        (error)=>{
                            ...
                            db.ref(`a-user/${userID}/onPassChange`).remove();
                        }
                    );
                }
            }
